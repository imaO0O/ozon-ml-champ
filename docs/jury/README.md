...
flowchart TB
    ...
    P2 --> P3
    P3 -->|"кривизна<br/>gamma = -0.00343"| FIN["<b>pair_q</b><br/>public 1.6466941<br/>первый слот"]
    ACC -.->|"второй слот"| FIN2["<b>pair_ac_cal</b><br/>public 1.6472270"]
...
Отвергнутых направлений сорок шесть, принятых четыре. Соотношение стоит
...
Плюс принятое 29.08: **смесь распределений** для вероятностного выхода —
CRPS +0.00159 и +0.00209 на двух срезах, и оптимальный масштаб стал общим
для срезов (1.08 против 1.12 и 1.16 у одной сети).
...
Разрыв с пятым местом — **0.0005508**, и это **меньше смещения отбора**:
...
(`σ_a ∝ σ_пары/γ`, а `σ_пары ∝ γ`). Поэтому кубический член калибровки
мы **не применили**, хотя он положителен: 5.1·10⁻⁶ при оценке 0.7σ. Знать,
чего прибор не различает, важнее, чем выжать из него ещё знак.

**3. Один закон объясняет весь состав.** «Расстояние ничего не значит,
значит направление» — не метафора, а формула `(D − δ)²/(4D)`, проверенная
шестью случаями в обе стороны. Из неё следует, почему сети различаются
**входом**, а не архитектурой; почему самый ценный партнёр **хуже** всех
остальных на 0.037; и почему обновление компоненты **навредило** составу.

**4. Отрицательных результатов больше, чем положительных, и они дороже.**
Сорок шесть направлений, каждое с числом и **с условием, при котором вывод
...
на 664 прогонах обоих треков. Пересборка отправленных файлов и основания
...
и мы показываем оба несогласных критерия вместо удобного. Воспроизводимость
на чужой машине не проверена, и мы этого не утверждаем.
