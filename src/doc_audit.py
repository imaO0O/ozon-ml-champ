...
несвязанных таблицах и превращалась бы в шум. Вердикт однозначен, как
у команд, — два разных числа об одном прогоне не бывают оба верны.
    ...
    for d in DOC_NUM:
        ...
        for line in t.split("\n"):
            ...
            for i, val in enumerate(parts[1:], 1):
                if i >= len(header) or not val:
                    continue
                key = (lab.group(1), header[i])
