...
# Параметры выбраны перебором на ПОЧИНЕННОЙ ранней остановке (24.08) и
# подтверждены на двух срезах против прежних умолчаний:
#
#     конфигурация                январь     декабрь
#     прежняя (127 листьев)       1.67002    1.73740
#     нынешняя (31 лист)          1.66936    1.73679
#                                 +0.00066   +0.00061
#
# Прежние умолчания были выбраны по перебору, который дефект остановки
# перевернул целиком: пока сравнение шло по сырому RMSLE, крупные деревья
# успевали набрать форму до обрыва, а мелкие нет, и 255 листьев стояли на
# первых местах. После починки порядок обратный и монотонный (PLAN, раздел
# про перевёрнутый перебор). Три сетки, 70 попыток, вершина на 31 листе.
#
# Осторожно с min_data_in_leaf=2000: на январе с ним делила первое место
# конфигурация с min_data_in_leaf=100, но на декабре она не дала ничего
# (−0.00003). Январская ничья не была ничьёй.
LGB_REG = dict(
...
# Классификатор НЕ наследует настроенные параметры регрессии. Перебор
# настраивал одноголовую регрессию log1p(target) — её метрика и есть метрика
# соревнования. У классификатора задача другая (купит или нет), и его
# оптимум перебором не проверялся ни разу.
#
# Цена наследования измерена. Когда 24.08 новые параметры регрессии
# протекли в классификатор через `{**LGB_REG}`, полный конвейер на январе
# дал 1.66931 против 1.66918 — то есть выигрыш регрессии +0.00066 не только
# не перенёсся, но ушёл в небольшой минус, а вес бленда прыгнул с 0.45
# на 0.70. Здесь параметры классификатора закреплены на тех, при которых
# конвейер и настраивался.
LGB_BIN = dict(
    objective="binary",
    metric="auc",
    learning_rate=0.03,
