...
# Умолчания конвейера. НЕ трогать по результатам перебора одиночной регрессии:
# перебор настраивает одну руку, а конвейер складывает одиночную регрессию
# с двухэтапной моделью, и лучшая одиночная рука не даёт лучшего конвейера.
# Замер 24.08 на январе, выровненный:
#
#     старые параметры целиком                     1.66918
#     настроенные в регрессии и классификаторе      1.66931
#     настроенные в регрессии, классификатор прежний 1.66943
#
# Механизм тот же, что и у мёртвого бленда (PLAN, про направление против
# расстояния): сильно зарегуляризованная регрессия глаже и потому БЛИЖЕ
# к двухэтапной модели. Партнёры сближаются, и бленд теряет больше, чем
# регрессия выигрывает поодиночке.
LGB_REG = dict(
...
# Победитель перебора на ПОЧИНЕННОЙ ранней остановке (24.08), подтверждённый
# на двух срезах против умолчаний — но только для ОДИНОЧНОЙ регрессии:
#
#     конфигурация              январь     декабрь
#     умолчания (127 листьев)   1.67002    1.73740
#     настроенная (31 лист)     1.66936    1.73679
#                               +0.00066   +0.00061
#
# Прежние умолчания выбирались по перебору, который дефект остановки
...
# первых местах. После починки порядок обратный и монотонный. Три сетки,
# 70 попыток, вершина на 31 листе — не на краю.
LGB_SINGLE_TUNED = {**LGB_REG, "num_leaves": 31, "min_data_in_leaf": 2000,
                    "feature_fraction": 0.5, "bagging_fraction": 1.0,
                    "lambda_l2": 1.0}
