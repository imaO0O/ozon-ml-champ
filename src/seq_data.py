...
def events_window(win: np.ndarray, n_tokens: int) -> np.ndarray:
    ...
    **-0.146**, то есть она не непохожий участник, а худшая копия сетки, и
    бленду не нужна ни в каком количестве.

    ОТОЗВАНО, 28.08: строка «год истории тоже не окупился — длина не была
    узким местом» держалась на сравнении с ДВУМЯ переменными. Событийных
    конфигураций в журнале ровно три, и переход e48 -> e96 менял бюджет
    токенов И окно одновременно:

        seq15x90e48    бюджет 48, окно 90
        seq15x365e96   бюджет 96, окно 365
        seq18x364e96   бюджет 96, окно 364

    Однофакторного замера длины среди них нет. Трек C поймал это у себя же,
    трек A подтвердил по журналу. Отказ направления при этом устоял: он
    держится на сравнении e48 с плотной сеткой (одна переменная —
    представление), на весе -0.146 в составе и на том, что train RMSE не
    сдвинулся. Недействителен только подпункт про длину истории.

    Открытый вопрос с числом: при бюджете 96 обрезается 40.11% клиентов
    (активность в годовом окне: медиана 76 дней, 90-й процентиль 202).
    Чистый опыт — e192 на том же окне 364, одна переменная.
