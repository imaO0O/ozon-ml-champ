...
Размер: 250 000 x 409 x 21 в float16 — около 4.0 ГБ на диске. Читается как
...
Каналы, 21 штука тремя группами:
...
* 6 дневных рангов среди активных этого дня (`rank_*`).
...
# Три первых канала — деньги, заказы, поиски — приняты замером на шести срезах.
# Три следующих добавлены по тому же правилу «добавлять уровнеустойчивый
# взгляд», и выбраны так, чтобы не дублировать первые три:
#
#   to_cart        намерение, а не покупка: клиент, который кладёт в корзину
#                  много относительно дня, но заказывает мало, ведёт себя иначе
#                  того, кто просто мал по объёму, — из to_ord это не видно;
#   search_to_ord  заказы, пришедшие из поиска,
#   cat_to_ord     заказы, пришедшие из каталога.
#
# Последние два расщепляют rank_ord по источнику. Доли каналов у нас уже есть,
# но это доли ВНУТРИ клиента, а здесь — положение клиента среди дня по каждому
# источнику отдельно, величина другая.
#
# Больше не берём намеренно. Оставшиеся каналы (gmv_search, gmv_cat, флаги)
# либо почти повторяют gmv по порядку, либо двоичны, и ранг по ним вырождается.
RANK_CHANNELS = ["rank_gmv", "rank_ord", "rank_searches",
                 "rank_cart", "rank_sord", "rank_cord"]
RANK_SOURCE = ["gmv", "to_ord", "searches",
               "to_cart", "search_to_ord", "cat_to_ord"]
