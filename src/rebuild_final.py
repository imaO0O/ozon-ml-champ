...
RECIPE = {
    ...
    **{f"evfin_s{s}": f"{SEQ} --lookback 364 --events 96 --static rk_ --bins 64 "
    ...
    # Прогон с чистого клона 31.08 показал, что тут не хватало ДВУХ шагов:
    # `predict.py` требует `models/netoof_b64a_test.npz` — предсказание сети
    # на ТЕСТОВОМ срезе, а `seq_oof_net.py` делает валидационные; тестовый
    # берётся флагом `--test-submission` из готового сабмита сети.
    #
    # Источник опознан прямым перебором, а не гипотезой. `--test-submission X`
    # делает ровно `pred_log = log1p(X.predict)`, значит `expm1(pred_log)`
    # обязан совпасть с одним из наших сабмитов до машинной точности. Сверка
    # по всем 204: `gru_b64a_avg3.csv`, max |Δ| 9.1e-13. Приём трека C.
    #
    # Гипотеза «это не среднее по сидам, у него дисперсия выше» была неверна:
    # сравнивались разные прогоны, а не среднее с одиночным. Прямой перебор
    # снял вопрос за секунды там, где рассуждение о дисперсии увело в сторону.
    "stk2_raw": ("четыре шага, порядок обязателен:\n"
                 ...
                 "log1p(f7)))\n"
                 "        сверено с нашим файлом: max |Δ| 1.1e-07\n"
                 "     3) python -u src/seq_oof_net.py --lookback 90 --static rk_ "
                 "--bins 64\n"
