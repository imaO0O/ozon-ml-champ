...
RECIPE = {
    ...
    **{f"evfin_s{s}": f"{SEQ} --lookback 364 --events 96 --static rk_ --bins 64 "
    ...
    "stk2_raw": ("python -u src/train.py --cutoffs 6 --ensemble --net "
                 ...
                 "--out stk2_raw.csv"),
    # Указатель «см. C3» стоял здесь ровно час и был неверен: в C3 этих
    # компонент нет. Тот же класс, который эта таблица и закрывала, — ссылка
    # написана без проверки цели. Ниже проверенные рецепты.
    "pair_ac": ("смесь половин пополам в log1p — expm1(0.5*log1p(mix_year) "
                "+ 0.5*log1p(cand_w2));\n     пара опознана перебором 12 сочетаний, "
                "лучшее впятеро точнее ближайшего (docs/jury/A2, редакция 29.08)"),
    "pair_ac_cal": ("выводится ЭТИМ ЖЕ скриптом из pair_ac (уровень, растяжение, "
                    "кривизна\n     GAMMA_AC) — запустите с --write, имея pair_ac"),
    "cand_w2_cal": ("артефакт трека C, у нас не воспроизводится: конфигурации\n"
                    "     и числа в docs/jury/C2_track_c.md и C3_reproduce.md"),
