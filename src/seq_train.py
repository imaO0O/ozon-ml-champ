...
def fit_cohorts(seq, train_rows, train_cuts, train_y, lookback, first_day,
    ...
    Дешёвой диагностики ДО обучения у нас нет, и записанная сначала неверна.
    Я утверждал, что база вредна, если `std` остатка не меньше `std` цели, —
    но std цели 2.2883, а у остатков 1.9653 и 1.9999, то есть обе базы разброс
    УМЕНЬШИЛИ и всё равно навредили. Уменьшения разброса недостаточно: важно
    не насколько остаток мал, а насколько он предсказуем, а это без обучения
    не видно. Правило снято.

    Механизм с нулями остаётся правдоподобным объяснением трёх отказов, но он
    именно объяснение, а не проверяемый заранее признак.
...
def train_model(seq, train_rows, train_y, train_cuts, val_rows, val_y, val_cut, args,
    ...
    # ВНИМАНИЕ: --epochs здесь не потолок, а ГОРИЗОНТ РАСПИСАНИЯ. OneCycle
    # отжигает скорость обучения ровно к последней эпохе из n_epochs, поэтому
    # два прогона с одинаковым сидом и разным --epochs — это разные обучения,
    # а не одно, обрезанное в разных местах.
    #
    # Замер, три сида, январь, выровненная метрика:
    #
    #     --epochs 12   1.66933 / 1.66954 / 1.66938   расписание завершилось
    #     --epochs 40   1.67132 / 1.66973 / 1.67063   оборвано остановкой на 18-22
    #
    # При 40 на двенадцатой эпохе скорость ещё высокая, кривая шумит, ранняя
    # остановка срабатывает до отжига — и модель не получает фазу тонкой
    # подстройки, ради которой цикл и нужен.
    #
    # Отсюда: ранняя остановка и OneCycle работают друг против друга. Правильно
    # подбирать длину расписания, а остановку держать настолько терпеливой,
    # чтобы цикл всегда доходил до конца.
    #
    # ДЛИНА ПОДОБРАНА, менять нечего. Остановка выключена, сид 42, январь:
    #
    #     8   1.66939      16   1.66923
    #     12  1.66933      24   1.67058
    #
    # От 8 до 16 разброс 0.00016 — внутри шума сида. При 24 хуже на 0.0013:
    # цикл растянут, и модель успевает переобучиться до отжига. Умолчание 12
    # лежит в плоской части, поднимать или опускать его смысла нет.
...
def main() -> None:
    ...
    # Терпение 8. ВАЖНО: прежнее обоснование снято, оно было артефактом.
    #
    # Мерили так: терпение 3 давало 1.67113, терпение 8 — 1.66968, +0.00146,
    # и вывод звучал как «мы полтора месяца недоучивали каждую сеть». Обе руки
    # при этом судились по СЫРОЙ метрике, в которую входит уровень. После её
    # починки терпение перестало быть узким местом: на потолке 12 эпох
    # выровненная кривая обновляет лучшую эпоху почти каждый раз, максимум два
    # неулучшения подряд, — то есть терпение 3 не сработало бы ни разу, и обе
    # руки дали бы одно и то же. Прибавка была не от терпения, а от того, что
    # больший запас давал кривой шанс вернуться после случайного скачка уровня.
    #
    # Восемь оставлено не ради прибавки, а как страховка: при длинном
    # расписании выровненная кривая всё-таки шумит (прогоны с --epochs 40
    # останавливались на 18-22 эпохе), и там запас нужен. Стоит он ничего.
    #
    # Декабрьский случай, где терпение 3 обрывало прогон на ПЕРВОЙ эпохе
    # (1.74312 против 1.73728 при терпении 6), — это, как заметил трек A, обе
    # болезни разом: запаса было мало, но отсчитывался он по величине, которая
    # у сети шумит сильнее из-за дрейфа уровня.
    ...
    # Финал: валидационный срез уходит в обучение, а горизонт расписания
    # берётся ТОТ ЖЕ, что на валидации, а не равный лучшей эпохе.
    #
    # Раньше здесь стояло epochs=best_epoch, и это ошибка того же рода, что
    # остальные наши: --epochs не число эпох, а ГОРИЗОНТ ОТЖИГА OneCycle.
    # Модель, отобранная как эпоха 9 из цикла в 12 (отжиг не завершён),
    # переобучалась полным циклом в 9 эпох — другой режим, а не тот же.
    #
    # Хуже того, у разных сидов лучшие эпохи разные, поэтому три усредняемых
    # финала обучались ТРЕМЯ РАЗНЫМИ расписаниями: у gru_dr90 циклами в 4, 5
    # и 8 эпох. Отсюда и разброс их уровней -0.034 / +0.054 / -0.029, который
    # мы списывали на сиды.
    #
    # Правильно так: длину расписания подбирают заранее (замер 8/12/16/24
    # выше по файлу), финал прогоняется ею целиком. Сведения о лучшей эпохе
    # при этом не пропадают: на починенной метрике она приходится на
    # последние эпохи, то есть цикл завершается вовремя.
    n_final = args.epochs
    print()
    ...
    final, _, _ = train_model(seq, all_rows, all_y, all_cuts, np.array([], dtype=int),
                              ...
                              epochs=n_final, train_base=all_base,
