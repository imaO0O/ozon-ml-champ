...
EXPERIMENT_FIELDS = [
    ...
    "rmsle_aligned",
    # Машина. Заведена после коллизии имён на кроссмашинной сверке: имя
    # прогона перестало определять прогон, как только один состав стали
    # гонять на разных картах. Значение проставляет `append_csv` сам —
    # разбор в `utils.machine_tag`. В старых строках пусто, и это не
    # «та же машина», а «не записано».
    "machine",
]
