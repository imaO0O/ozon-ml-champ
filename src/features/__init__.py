...
def build_target(cutoff: dt.date, lf: pl.LazyFrame | None = None) -> pl.DataFrame:
    ...
    **Движок здесь `in-memory`, и это не мелочь.** На `streaming` две сборки
    одной и той же разметки расходятся: замер 30.08 дал 7.3e-12 максимум
    по 250 000 клиентов — порядок суммирования внутри группы недетерминирован.
    Признаки при этом побитово стабильны (243 колонки, две сборки, ноль
    расхождений), то есть невоспроизводима была именно РАЗМЕТКА.

    Почему 7e-12 стоит переключения движка: бустинг к такому различию
    чувствителен непропорционально. Уже измерено — та же конфигурация после
    пересборки кэша дала 1.68551 вместо 1.68637 на 144-й итерации вместо
    152-й, то есть **0.00086 RMSLE от расхождения в двенадцатом знаке**.
    Именно поэтому критерий приёмки у нас 0.001, а не побитовое совпадение.
    С `in-memory` разметка воспроизводится побитово, и этот источник
    расхождения исчезает целиком.

    Цена перехода нулевая: 0.05 с против 0.04 с на полном логе — окно цели
    короткое, тридцать дней, и потоковый режим тут ничего не экономил.
    Признаки собираются на всей истории и остаются на `streaming` намеренно:
    там движок нужен по памяти, а детерминизм и так есть.
    """
    lf = scan_log() if lf is None else lf
