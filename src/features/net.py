...
**Уровень сети по умолчанию не подаётся.** Среднее её предсказаний скачет между
срезами от 2.149 до 2.475, а на тесте равно 2.379 — размах 0.32 при стандартном
отклонении остатка около 1.65. Абсолютное значение означает разное на разных
срезах, и деревья могут выучить по нему сам срез. Ровно та же болезнь, что
лечит блок `ranks`.
...
**Решение проверено, а не принято на веру** (флаг `--net-feats`). Трек C
заметил, что у него стекинг на сыром предсказании давал вчетверо больше нашего,
и предположил, что вместе с шумом уровня мы выбросили сигнал: уровень
предсказания сети может нести информацию о клиенте, а не только о срезе.
Числа замера — PLAN.md, раздел 5.

Важно при чтении результатов: `raw` **рядом с** `centered` — это не «плюс
уровень клиента», а «плюс среднее по срезу», потому что их разность равна
ему в точности. Поэтому наборы разведены: `raw` подаёт сырое предсказание
вместо пары, а не вместе с ней.
...
FEATS = ("rank_centered", "raw", "all")


def attach(df: pl.DataFrame, cutoff: dt.date, test_cutoff: dt.date,
           names: list[str] | None = None, feats: str = "rank_centered") -> pl.DataFrame:
    """Добавить признаки каждой сети; `feats` выбирает их набор.

    rank_centered — ранг внутри среза и предсказание минус среднее по срезу
                    (умолчание: обе величины не зависят от уровня среза);
    raw           — сырое предсказание как есть, вместе с уровнем;
    all           — и то, и другое.

    Про `raw` см. предупреждение в шапке модуля: подавая его рядом с
    центрированным, вы фактически даёте деревьям и среднее по срезу —
    их разность равна ему в точности.
    """
    if feats not in FEATS:
        raise SystemExit(f"неизвестный набор признаков сети '{feats}'; есть: {', '.join(FEATS)}")
    ...
    for name in names:
        ...
        if feats in ("raw", "all"):
            cols.append(pl.col("_net_pred").cast(pl.Float32).alias(f"net{suffix}_raw"))
        out = out.with_columns(cols).drop("_net_pred")
    return out
