...
## Имена наборов: дефект должен стоять в имени

Набор `b64` переименован 29.08 в **`b64stop2`**, и цифра в имени — это
номер эпохи, на которой обрывались его сети. Он сделан 20.08, до починки
ранней остановки по выровненной метрике; сверка восемнадцати прогонов
показала, что там, где выбранная эпоха совпала с нынешней, предсказания
совпадают побитово, а где разошлась — расходятся до 4.17 в log1p. Старые
прогоны останавливались на **2-й и 4-й эпохе из двадцати**.

Почему переименован, а не удалён: набор доказывает величину той починки
(+0.00112) и служит материалом побитовой сверки. Почему переименован,
а не оставлен как есть: **README велел жюри стекать именно на `b64`**,
и инструкция воспроизводила предшественника на недоученных сетях. Дефект
прятался не в тексте, а в имени файла, и его не поймала ни одна сверка
чисел — только сквозной прогон.

Рабочий набор состава — **`b64a`** (24.08, после починки), проверено
по `lgbm_stk2_meta.json`.

Теперь `--net-names b64` падает громко: `attach` бросает `SystemExit`
на отсутствующем файле, а не подхватывает молча что-то другое.
"""
