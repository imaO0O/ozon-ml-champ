...
распределение) и даёт три независимых улучшения:
    ...
    CRPS                       январь +0.00174, декабрь +0.00318
    оптимальный масштаб        1.08 на ОБОИХ срезах (было 1.12 и 1.16)
    покрытие веса состава      100% (четыре компоненты, включая сеть
                               под стекинг, входящую через бустинг)

Разрыв между критериями масштаба смесь НЕ чинит: 0.44 у одной сети,
0.40 у смеси трёх, снова 0.44 у смеси четырёх. Полнота покрытия на форму
распределения не влияет, причина противоречия остаётся не найденной.
